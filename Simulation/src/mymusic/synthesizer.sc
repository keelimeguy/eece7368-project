...
behavior GenWaveform(in uint16_t freq_values[MAX_CHORD_SIZE],
                     in bool playing,
                     in uint8_t volume,
                     AudioWriter audio,
                     event error,
    ...
    void main() {
        ...
        while (1) {
            ...
            if (count < MAX_CHORD_SIZE - 1) count++;
            else count = 0;
...
behavior Synthesizer(i_receiver synth_stream, ChordWriter out_stream, event error,
    ...
    ParseChord parse_chord(synth_stream, out_stream, freq_values, playing, volume);
    GenWaveform gen_waveform(freq_values, playing, volume, audio, error, stop);
